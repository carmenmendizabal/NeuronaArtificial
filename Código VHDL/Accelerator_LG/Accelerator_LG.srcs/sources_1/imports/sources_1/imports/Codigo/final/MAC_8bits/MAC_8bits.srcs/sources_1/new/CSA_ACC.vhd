-- modulo basado en sumador de:
-- https://github.com/sromerop/ArtificialNeuron

library IEEE;
use IEEE.STD_LOGIC_1164.ALL;
use work.csa_acc_utils.all;


entity CSA_ACC is
    generic( ninputs : integer;
			 ncycles : integer);
    Port ( a : in STD_LOGIC_VECTOR ((nbits - 1) downto 0); -- señal procedente del árbol de sumadores
           b : in STD_LOGIC_VECTOR ((nbits + ncycles + (nbits_RCA - ((nbits + ncycles) rem nbits_RCA)) - 1) downto 0); -- señal realimentada
           cin : in STD_LOGIC;
           s : out STD_LOGIC_VECTOR ((nbits + ncycles + (nbits_RCA - ((nbits + ncycles) rem nbits_RCA)) - 1) downto 0); -- señal de salida de ancho variable
           cout : out STD_LOGIC);
end CSA_ACC;

architecture Behavioral of CSA_ACC is
...
constant remaining : integer := (nbits_RCA - ((nbits + ncycles) rem nbits_RCA));  -- in order to fill all the inputs of the RCAs
signal zeros_a : STD_LOGIC_VECTOR ((ncycles - 1) downto 0) := (others=>'0');    -- a and b with the same width (difference factor)
signal ones_a : STD_LOGIC_VECTOR ((ncycles - 1) downto 0) := (others=>'1');    -- a and b with the same width (difference factor)
signal addend_a : STD_LOGIC_VECTOR ((nbits + ncycles - 1) downto 0); -- a and b with the same width
signal addend_b : STD_LOGIC_VECTOR ((nbits + ncycles + remaining - 1) downto 0); -- a and b with the same width
...
signal addend_a_fin, addend_b_fin : STD_LOGIC_VECTOR ((nbits + ncycles + remaining - 1) downto 0);    -- final width of the addends (same width and filling all the inputs of the RCAs)

-- intermediate signal handling
constant nMUX : integer := nbits + ncycles + remaining + 1;  -- word width plus carry-out (+1)
constant nRCA : integer := 2*((nbits + ncycles)/nbits_RCA + 1);   -- sum is processed twice using ceiling function (+1)
...
signal sum_output : STD_LOGIC_VECTOR ((nbits + ncycles + remaining - 1) downto 0);
...
addend_a <= zeros_a & a when a(nbits - 1) = '0' else  -- sign extension
            ones_a & a;
addend_b <= b;
...
-- sum of the adjusted inputs
    ...
    RCA_N4N1_1: RCA_4b    -- carry-in bit is 1
        port map(a => addend_a_fin((nbits + ncycles + remaining - 1) downto (nbits + ncycles + remaining - nbits_RCA)),
                 b => addend_b_fin((nbits + ncycles + remaining - 1) downto (nbits + ncycles + remaining - nbits_RCA)),
        ...
        for I in 0 to (nbits_RCA - 1) generate
           MUX_X: mux_2to1  -- MUX to select the final value of the sum
               port map(a => sum_0(nRCA/2 - 1)(I),
                        b => sum_1(nRCA/2 - 1)(I),
                        ctrl => cin_input,
                        s => sum_output(nbits + ncycles + remaining - nbits_RCA + I));
