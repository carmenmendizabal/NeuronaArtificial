...
component CSA_ACC
    ...
    Port ( a : in STD_LOGIC_VECTOR ((nbits - 1) downto 0); -- input that comes from the multiplier
           b : in STD_LOGIC_VECTOR ((nbits + ncycles + (nbits_RCA - ((nbits + ncycles) rem nbits_RCA)) - 1) downto 0); -- input that comes from the accumulator
           cin : in STD_LOGIC;  -- carry-in bit
           s : out STD_LOGIC_VECTOR ((nbits + ncycles + (nbits_RCA - ((nbits + ncycles) rem nbits_RCA)) - 1) downto 0); -- output that goes to the accumulator
